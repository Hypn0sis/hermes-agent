...
class DiscordAdapter(BasePlatformAdapter):
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        # Check extra config first, then fall back to env var
        config_auto_thread = self.config.extra.get("auto_thread")
        if config_auto_thread is not None:
            if config_auto_thread == "smart":
                # Smart mode: only create thread when explicitly mentioned in a public channel
                # In DMs and threads, no auto-thread
                auto_thread = False
            else:
                auto_thread = bool(config_auto_thread)
        else:
        ...
        # Smart mode: don't auto-create thread in DMs or if not mentioned
        if (
            config_auto_thread == "smart"
            and not is_thread
            and not isinstance(message.channel, discord.DMChannel)
        ):
            # Check if message was a mention (explicit @bot)
            mention_found = self._client.user and self._client.user in message.mentions
            if mention_found:
                # Mentioned - create thread
                auto_thread = True
            else:
                # Not mentioned - don't create thread, respond directly
                auto_thread = False
